...
def projectedBacktrackingSearch(f, P, x: np.array, d: np.array, sigma=1.0e-4, verbose=0, max_iterations=100):
    xp = P.project(x)
    gradx = f.gradient(xp)
    decrease = gradx.T @ d
    ...
    def W(t):
        xa = P.project(x + t * d)
        return f.objective(xa) <= f.objective(x) - ((sigma / t) * (np.linalg.norm(x - P.project(x - t * f.gradient(x))) ** 2))
