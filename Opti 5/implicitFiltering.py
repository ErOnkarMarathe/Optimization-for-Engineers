...
def derivativefreeBFGSDescent(f, x0: np.array, h, eps=1.0e-3, sigma=1.0e-4, verbose=0):
    ...
    while not satisfiesTermination:
        betaK = min(1, 10 * h / np.linalg.norm(B @ grad_f_h))
        dk = -betaK * B @ grad_f_h

        t = 1
        linesearchCounter = 0
        while f.objective(x + t * dk) > f.objective(x) + sigma * t * grad_f_h.T @ dk:
            t = 0.5 * t
            linesearchCounter += 1
        ...
        if linesearchFail:
            ...
            deltagk = SUC.SUCSimplexGradient(f, x + t * dk, h) - grad_f_h
            deltaxk = t * dk

            x = x + deltaxk

            update_term1 = np.outer(deltaxk, deltaxk) / (deltaxk.T @ deltagk)
            update_term2 = B @ deltagk @ deltagk.T @ B / (deltagk.T @ B @ deltagk)

            B = B + update_term1 - update_term2
    ...
    if verbose:
        print('derivativefreeBFGSDescent terminated after ', loopCounter, ' steps with norm of gradient =',
              np.linalg.norm(grad_f_h))
